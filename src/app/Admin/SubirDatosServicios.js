...
export const SubirEncabezado = () => {
  ...
  useEffect(() => {
    const fetchEncabezado = async () => {
      try {
        const docRef = doc(db, "texto", "apartado0");
...
export const SubirDatosServicios1 = () => {
...
export const SubirDatosServicios2 = () => {
  const [forms, setForms] = useState([{ id: Date.now(), title: '', contenido: '', imageFile: null }]);
  const [loading, setLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [message, setMessage] = useState('');
...
export const SubirDatosServicios3 = () => {
  const [forms, setForms] = useState([{ id: Date.now(), title: '', contenido: '', imageFile: null }]);
  const [loading, setLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [message, setMessage] = useState('');
