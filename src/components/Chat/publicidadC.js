...
export const ShopNowButtonP = () => {
  ...
  // Manejo de la entrada de URL
  const handleUrlChange = (e) => {
    setImageUrl(e.target.value);
  };

  // Función para manejar la subida a Firebase Storage y Firestore
  const handleUpload = async () => {
    setUploading(true);
    let imageUrlToUpload = imageUrl;

    // Si hay una imagen seleccionada, subirla a Storage y luego guardar la URL en Firestore
    if (selectedImage) {
      ...
      uploadTask.on(
        'state_changed',
        null, // Puedes manejar el progreso aquí si lo deseas
        (error) => {
        ...
        async () => {
          ...
          try {
            // Aquí se usa la colección "publicidadP"
      ...
      // Si solo se ingresa una URL, se guarda en la colección "publicidadP"
      try {
        await addDoc(collection(db, 'publicidadP'), {
          imageUrl: imageUrlToUpload,
          timestamp: new Date(),
          ...
          <div className="bg-white p-6 rounded-md shadow-lg w-96">
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                disabled={uploading}
                className={`${uploading ? 'bg-gray-500' : 'bg-blue-500'} text-white px-4 py-2 rounded-md`}
              >
